...
public class AuthService {
    ...
    public AuthResponse register(RegisterRequest req) {
        if (req.getPhone() == null || !req.getPhone().matches("^[6-9]\\d{9}$")) {
            throw new RuntimeException("Invalid Indian mobile number");
        }
        if (req.getPassword() == null || req.getPassword().length() < 8 ||
            !req.getPassword().matches(".*[A-Z].*") ||
            !req.getPassword().matches(".*[a-z].*") ||
            !req.getPassword().matches(".*\\d.*")) {
            throw new RuntimeException("Password must be at least 8 characters long, deeply complex with uppercase, lowercase, and numbers");
        }
    ...
    public void requestPasswordReset(String email) {
        ...
        // Identity Anchor: Always use the database-stored email normalized to lowercase for the vault key
        String canonicalEmailKey = user.getEmail().toLowerCase().trim();
        String generatedOtp = String.format("%06d", SECURE_RANDOM.nextInt(1_000_000));
        
        try {
            System.out.println(">>> PASSWORD RESET OTP FOR [" + canonicalEmailKey + "]: " + generatedOtp);
            emailService.sendCode(user.getEmail(), generatedOtp);
            otpVault.put(canonicalEmailKey + "_RESET", new PendingRegistration(null, generatedOtp));
        } catch (Exception e) {
            System.err.println("Password reset email failure: " + e.getMessage());
    ...
    public void resetPassword(String email, String otp, String newPassword) {
        User user = userRepo.findByEmailIgnoreCase(email.trim())
                .orElseThrow(() -> new RuntimeException("Identity resolution failed: User not found."));
        
        String canonicalEmailKey = user.getEmail().toLowerCase().trim();
        String normalizedOtp = otp.trim();
        String vaultKey = canonicalEmailKey + "_RESET";
        
        PendingRegistration pending = otpVault.get(vaultKey);
        
        if (pending == null || pending.isExpired()) {
            otpVault.remove(vaultKey);
            throw new RuntimeException("Reset request expired or not found. Please initiate again.");
        }
        
        if (!pending.otp.equals(normalizedOtp)) throw new RuntimeException("Invalid Password Reset OTP");

        if (newPassword == null || newPassword.length() < 8 ||
            !newPassword.matches(".*[A-Z].*") ||
            !newPassword.matches(".*[a-z].*") ||
            !newPassword.matches(".*\\d.*")) {
            throw new RuntimeException("Password security policy violation: Does not meet complexity requirements.");
        }
