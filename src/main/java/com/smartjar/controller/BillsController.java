...
public class BillsController {
    ...
    public BillsController(com.smartjar.repository.WalletRepository walletRepo, 
                           ...
                           com.smartjar.repository.UserRepository userRepo) {
    ...
    public Map<String, String> payBill(@RequestBody Map<String, Object> req) {
        ...
        String billerNumber = (String) req.get("billerNumber");
        ...
        com.smartjar.entity.User user = userRepo.findById(userId)
        ...
        tx.setSenderDetails(user.getName());
        tx.setReceiverDetails(billerName + " (" + billerNumber + ")");
        ...
        if (totalSavingsToExecute.compareTo(java.math.BigDecimal.ZERO) > 0 && wallet.getBalance().compareTo(totalSavingsToExecute) >= 0) {
            ...
            com.smartjar.entity.Transaction goldTx = new com.smartjar.entity.Transaction();
            goldTx.setSenderId(userId);
            goldTx.setReceiverId(userId);
            goldTx.setAmount(totalSavingsToExecute);
            goldTx.setTransactionType("GOLD_SAVINGS_SWEEP");
            goldTx.setSenderDetails("Utility Hub Autopilot");
        ...
        if (frequency != null && !frequency.isEmpty() && !frequency.equals("ONCE")) {
            ...
            recur.setBillerNumber(billerNumber);
    ...
    @PostMapping("/autopay/{id}/toggle")
    public Map<String, String> toggleAutopay(@PathVariable java.util.UUID id) {
        com.smartjar.entity.RecurringBill recur = recurringRepo.findById(id)
            .orElseThrow(() -> new RuntimeException("Autopay instruction not found"));
        recur.setActive(!recur.isActive());
        recurringRepo.save(recur);
        Map<String, String> res = new HashMap<>();
        res.put("status", "SUCCESS");
        res.put("message", "Autopay is now " + (recur.isActive() ? "ACTIVE" : "PAUSED"));
        return res;
    }

    @DeleteMapping("/autopay/{id}")
    public Map<String, String> stopAutopay(@PathVariable java.util.UUID id) {
        recurringRepo.deleteById(id);
        Map<String, String> res = new HashMap<>();
        res.put("status", "SUCCESS");
        res.put("message", "Autopay permanently stopped and deleted");
        return res;
    }

    @GetMapping("/autopay/{userId}")
    public java.util.List<com.smartjar.entity.RecurringBill> getAutopayments(@PathVariable java.util.UUID userId) {
        // Find all by userId. Assuming we don't have a specific method in repo, we might need one or fetch all and filter.
        // But let's assume repository supports it or we filter manually for safety here if custom method missing.
        return recurringRepo.findAll().stream().filter(b -> b.getUserId().equals(userId)).toList();
    }
}
