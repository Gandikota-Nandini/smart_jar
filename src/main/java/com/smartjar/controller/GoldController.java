...
public class GoldController {
    ...
    public GoldController(WalletRepository walletRepo, GoldHoldingRepository goldRepo, TransactionRepository txRepo) {
    ...
    public Map<String, Object> buyGold(@RequestBody Map<String, Object> req) {
        ...
        BigDecimal grams = amountInr.divide(livePrice, 6, RoundingMode.HALF_UP);

        wallet.setBalance(wallet.getBalance().subtract(amountInr).setScale(2, RoundingMode.HALF_UP));
        ...
        tx.setAmount(amountInr);
        tx.setTransactionType("GOLD_PURCHASE");
        tx.setSenderDetails("Internal Wallet");
        tx.setReceiverDetails("Digital Gold Vault");
    ...
    public Map<String, Object> sellGold(@RequestBody Map<String, Object> req) {
        ...
        if (totalGramsOwned.compareTo(gramsToSell) < 0) {
            if (gramsToSell.subtract(totalGramsOwned).compareTo(new BigDecimal("0.0001")) <= 0) {
                // If it's a tiny rounding discrepancy from frontend, sell all
                gramsToSell = totalGramsOwned;
            } else {
        ...
        wallet.setBalance(wallet.getBalance().add(cashValue).setScale(2, RoundingMode.HALF_UP));
        walletRepo.save(wallet);
        ...
        tx.setTransactionType("GOLD_LIQUIDATION");
        tx.setSenderDetails("Digital Gold Vault");
