...
export default function SendMoney() {
    const [mode, setMode] = useState('UPI'); // 'UPI' or 'BANK'
    ...
    const verifyUpi = async () => {
        if(mode === 'UPI' && !upiId) return;
        if(mode === 'BANK' && (!bankAccount || !ifsc)) return;
        setLoading(true);
        try {
            let res;
            if (mode === 'UPI') {
                res = await api.get(`/upi/verify/${upiId}`);
            } else {
                res = await api.get(`/upi/verify/bank?account=${bankAccount}&ifsc=${ifsc}`);
            }
            
            if (res.data.valid === false) {
                alert(res.data.error || "Verification Failed: Invalid details or domain not recognized.");
                return;
            }
            ...
            alert("Network Error: Could not reach verification servers.");
        } finally {
            setLoading(false);
        }
    }
    ...
    const confirmPayment = async () => {
        ...
        try {
            ...
            const targetUpi = receiver.upiId; // the mock upiId generated in our verify step
                ...
                <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
                    {step === 1 && (
                        <div className="space-y-4">
                            <h2 className="text-xl font-black text-slate-900">Transfer Funds</h2>
                            
                            <div className="flex bg-slate-100 p-1 rounded-xl mb-4">
                                <button onClick={() => setMode('UPI')} className={`flex-1 text-sm font-bold py-2 rounded-lg transition-all ${mode === 'UPI' ? 'bg-white shadow text-indigo-600' : 'text-slate-500'}`}>UPI ID</button>
                                <button onClick={() => setMode('BANK')} className={`flex-1 text-sm font-bold py-2 rounded-lg transition-all ${mode === 'BANK' ? 'bg-white shadow text-indigo-600' : 'text-slate-500'}`}>Bank Account</button>
                            </div>
                            ...
                            {mode === 'UPI' ? (
                                ...
                                <div className="space-y-3">
                                    <input type="text" placeholder="Enter Account Number" className="w-full p-4 border rounded-xl font-medium" 
                                        value={bankAccount} onChange={e => setBankAccount(e.target.value)} />
                                    <input type="text" placeholder="Enter IFSC Code" className="w-full p-4 border rounded-xl font-medium uppercase" 
                                        value={ifsc} onChange={e => setIfsc(e.target.value.toUpperCase())} />
                                </div>
                            )}

                            <button onClick={verifyUpi} disabled={loading || (mode === 'UPI' ? !upiId : (!bankAccount || !ifsc))} className="w-full bg-indigo-600 text-white font-bold p-4 rounded-xl hover:bg-indigo-700 disabled:opacity-50">
                                {loading ? 'Verifying...' : (mode === 'UPI' ? 'VERIFY UPI ID' : 'VERIFY BANK ACCOUNT')}
                            </button>
                        </div>
                    )}
