...
import { ArrowLeft, Zap, Wifi, Tv, Droplet, Home, CheckCircle, Clock, Smartphone } from 'lucide-react';
...
export default function Bills() {
    ...
    useEffect(() => {
        if(user) {
            api.get(`/bills/autopay/${user.id}`).then(res => setAutopays(res.data)).catch(() => {});
        }
    }, []);

    const billers = [
        { id: 'elec', name: 'Electricity Board', icon: <Zap className="text-amber-500 w-8 h-8"/>, bg: 'bg-amber-100' },
        { id: 'mobile', name: 'Mobile Recharge', icon: <Smartphone className="text-pink-500 w-8 h-8"/>, bg: 'bg-pink-100' },
    ...
    const handlePayBill = async () => {
        ...
        setLoading(true);
        try {
            const defaultRules = [{ id: 1, active: true }, { id: 2, active: false }, { id: 3, active: false }];
    ...
    const handleToggleAutopay = async (id) => {
        try {
            await api.post(`/bills/autopay/${id}/toggle`);
            setAutopays(autopays.map(a => a.id === id ? {...a, active: !a.active} : a));
        } catch(e) {
            alert("Failed to toggle autopay");
        }
    };

    const handleStopAutopay = async (id) => {
        if (!window.confirm("Are you sure you want to permanently delete this Auto-Pay schedule?")) return;
        try {
            await api.delete(`/bills/autopay/${id}`);
            setAutopays(autopays.filter(a => a.id !== id));
            alert("Autopay successfully deleted.");
        } catch(e) {
            alert("Failed to delete autopay");
        }
    };
    ...
    return (
        <div className="p-4 pt-24 md:pt-10 w-full animate-[fadeIn_0.3s_ease]">
            <div className="bg-white max-w-2xl mx-auto rounded-3xl shadow-xl overflow-hidden border border-slate-100">
                <div className="bg-slate-900 p-6 flex flex-col items-center justify-center text-white text-center">
                ...
                <div className="p-8">
                        ...
                        <div className="space-y-6 animate-[fadeIn_0.3s_ease]">
                            ...
                            {autopays.length > 0 && (
                                <div className="mt-8 border-t border-slate-100 pt-6">
                                    <h3 className="text-slate-500 font-bold tracking-widest text-xs uppercase mb-4">Active Autopayments</h3>
                                    <div className="space-y-3">
                                        {autopays.map(a => (
                                            <div key={a.id} className="flex justify-between items-center p-4 bg-slate-50 rounded-xl border border-slate-100">
                                                <div>
                                                    <p className="font-bold text-slate-900 text-sm">{a.billerName} <span className="text-slate-500 font-medium">({a.frequency})</span></p>
                                                    <p className="text-xs font-bold text-slate-400 mt-0.5">₹{a.amount} • {a.billerNumber || 'No Ref'} • Next: {a.nextRunDate}</p>
                                                </div>
                                                <div className="flex space-x-2">
                                                    <button onClick={() => handleToggleAutopay(a.id)} className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all ${a.active ? 'bg-rose-100 text-rose-600 hover:bg-rose-200' : 'bg-emerald-100 text-emerald-600 hover:bg-emerald-200'}`}>
                                                        {a.active ? 'Pause' : 'Resume'}
                                                    </button>
                                                    <button onClick={() => handleStopAutopay(a.id)} className="px-3 py-1.5 rounded-full text-xs font-bold bg-slate-200 text-slate-700 hover:bg-slate-300 transition-all">
                                                        Stop
                                                    </button>
                                                </div>
                                            </div>
                    ...
                    {step === 2 && selectedBiller && (
                        <div className="space-y-6 animate-[fadeIn_0.3s_ease]">
                            ...
                            <div>
                                <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Receiver / Account Number</label>
                                <input type="text" placeholder="e.g. Phone or Consumer No." className="w-full p-4 border-2 border-slate-200 rounded-xl text-lg font-bold text-slate-900 focus:border-slate-900 focus:ring-0 transition-colors bg-slate-50 focus:bg-white"
                                       value={billerNumber} onChange={e => setBillerNumber(e.target.value)} />
                            </div>

                            <div>
                                <label className="block text-xs font-bold text-slate-500 uppercase mb-2 mt-4">Invoice Amount (₹)</label>
